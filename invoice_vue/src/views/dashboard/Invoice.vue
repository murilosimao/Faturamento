    ...
    <div class="page-invoice">
        <nav class="breadcrumb" aria-label="breadcrumbs">
            <ul>
                ...
                <li><router-link to="/dashboard/invoices">Faturas</router-link></li>
        ...
        <div class="columns is-multiline">
            <div class="column is-12">
                <h1 class="title">Fatura - {{ invoice.invoice_number }}</h1>

                <div class="buttons">
                    <button @click="getPdf()" class="button is-dark">Baixar PDF</button>

                    <template v-if="!invoice.is_credit_for && !invoice.is_credited">
                        <button @click="setAsPaid()" class="button is-success" v-if="!invoice.is_paid">Marcar como paga</button>
                        <button @click="createCreditNote()" class="button is-danger" v-if="!invoice.is_paid">Criar nota de crédito</button>
                    </template>

                    <button @click="sendReminder()" class="button is-info" v-if="!invoice.is_paid && !invoice.is_credit_for">Enviar lembrete</button>
                </div>
            </div>

            <div class="column is-12 mb-4">
                <div class="box">
                    <h3 class="is-size-4 mb-5">Cliente</h3>
            ...
            <div class="column is-12 mb-4">
                <div class="box">
                    <h3 class="is-size-4 mb-5">Itens</h3>
            ...
            <div class="column is-12">
                <div class="box">
                    <h3 class="is-size-4 mb-5">Resumo</h3>

                    <div class="columns">
                        <div class="column is-6">
                            <p><strong>Valor líquido</strong>: {{ invoice.net_amount }}</p>
                            <p><strong>Imposto</strong>: {{ invoice.vat_amount }}</p>
                            <p><strong>Valor bruto</strong>: {{ invoice.gross_amount }}</p>
                            <p><strong>Conta do banco</strong>: {{ invoice.bankaccount }}</p>
                        </div>
                    
                        <div class="column is-6">
                            <p><strong>Nossa Referência</strong>: {{ invoice.sender_reference }}</p>
                            <p><strong>Referência do cliente</strong>: {{ invoice.client_contact_reference }}</p>
                            <p><strong>Dias para vencer</strong>: {{ invoice.get_due_date_formatted }}</p>
                            <p><strong>Status</strong>: {{ getStatusLabel() }}</p>
                            <p><strong>Tipo de fatura</strong>: {{ getInvoiceType() }}</p>
                        </div>
                    </div>
                </div>
            </div>
